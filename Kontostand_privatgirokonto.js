fetch('Kontostand_privatgirokonto.txt')
    .then(response => {
        if (!response.ok) {
    ...
    .then(text => {
        ...
        document.getElementById('Kontostand_privatgirokonto').textContent = formattedText;
    })
    .catch(error => {
        document.getElementById('Kontostand_privatgirokonto').textContent = 'Fehler beim Laden der Datei: ' + error;
    });
